...
const [AsyncWaterfall, AsyncApply, AsyncEach] = [
...
describe('Database', () => {
        ...
        [
          ...
          function (cb) {
            ...
            d.find({}, (err, docs) => {
              ...
              _.pluck(docs, 'somedata').should.contain('ok');
              _.pluck(docs, 'somedata').should.contain('another');
              _.find(docs, (d) => d.somedata === 'another').plus.should.equal('additional data');
              _.pluck(docs, 'somedata').should.contain('again');
              return cb();
            });
          },
  ...
  describe('Update', () => {
    ...
    it('If timestampData option is set, update the updatedAt field', (done) => {
      ...
      d.insert({ hello: 'world' }, (err, insertedDoc) => {
        ...
        setTimeout(() => {
          ...
          d.update({ _id: insertedDoc._id }, { $set: { hello: 'mars' } }, {}, () => {
    ...
    describe('Upserts', () => {
      ...
      it('If the update query contains $setOnInsert, works as expected', (done) => {
        d.update(
          {},
          { $set: { hello: 'world1' }, $inc: { bloup: 3 }, $setOnInsert: { world: 'hello' } },
          { upsert: true },
          (err) => {
            d.find({ hello: 'world1' }, (err, docs) => {
              assert.isNull(err);
              docs.length.should.equal(1);
              const doc = docs[0];
              Object.keys(doc).length.should.equal(4);
              doc.hello.should.equal('world1');
              doc.world.should.equal('hello');
              doc.bloup.should.equal(3);
              done();
            });
          }
      ...
      it('Indexes are persisted with their options and recreated even if some db operation happen between loads', (done) => {
        ...
        db.insert({ planet: 'Earth' }, (err) => {
          ...
          db.insert({ planet: 'Mars' }, (err) => {
            ...
            db.ensureIndex({ fieldName: 'planet', unique: true, sparse: false }, (err) => {
              Object.keys(db.indexes).length.should.equal(2);
              Object.keys(db.indexes)[0].should.equal('_id');
              Object.keys(db.indexes)[1].should.equal('planet');
              db.indexes._id.getAll().length.should.equal(2);
              ...
              db.insert({ planet: 'Jupiter' }, (err) => {
                ...
                db.loadDatabase((err) => {
                  ...
                  db.ensureIndex({ fieldName: 'bloup', unique: false, sparse: true }, (err) => {
                    assert.isNull(err);
