...
describe('Model', () => {
  ...
  describe('Deep copying', () => {
    ...
    it('With the strictKeys option, only valid keys gets deep copied', () => {
      ...
      const b = model.deepCopyStrictKeys(a);
      assert.deepEqual(b, { a: 4, nested: { yes: 1 }, array: [{}, { yes: true }, {}] });
    });

    it('With the strictKeys option, $fn should not be copied', () => {
      const a = { $fn() {} };
      const b = model.deepCopyStrictKeys(a);
      assert.deepEqual(b, {});
    });
  ...
  describe('Modifying documents', () => {
    ...
    describe('$set modifier', () => {
      ...
      it('Can set sub-fields and create them if necessary', () => {
        ...
        const updateQuery = {
          $set: { 'yup.subfield': 'changed', 'yup.yop': 'yes indeed', 'totally.doesnt.exist': 'now it does' },
        };
        const modified = model.modify(obj, updateQuery);
      ...
      it('Can update nested fields in an array', () => {
        const obj = { nested: [{ name: 'old' }] };
        const updateQuery = { $set: { 'nested.0.name': 'new' } };
        const modified = model.modify(obj, updateQuery);

        assert.deepEqual(modified, { nested: [{ name: 'new' }] });
      });

      it('Can update all nested fields in an array', () => {
        const obj = { nested: [{ name: 'old1' }, { name: 'old2' }] };
        const updateQuery = { $set: { 'nested.$.name': 'new' } };
        const modified = model.modify(obj, updateQuery);

        assert.deepEqual(modified, { nested: [{ name: 'new' }, { name: 'new' }] });
      });

      it('Can update primitive fields in an array', () => {
        const obj = { nested: [1, 2, 3, 4] };
        const updateQuery = { $set: { 'nested.2': 2 } };
        const modified = model.modify(obj, updateQuery);

        assert.deepEqual(modified, { nested: [1, 2, 2, 4] });
      });
    }); // End of '$set modifier'
    ...
    describe('$unset modifier', () => {
      ...
      it('When unsetting nested fields in array, should work as expected', () => {
        const src = {
          title: 'subset 1',
          author: 'me',
          _id: '5f82390dd631f114d29f40c8',
          def: 'kandinsky',
          comments: [
            {
              user: 'user1',
              title: 'first comment',
              date: '2020-10-10T22:43:25.228Z',
              comments: [],
            },
            {
              user: 'user2',
          ...
          tags: [],
          numbers: [],
          meta: { date: '2020-10-10T22:43:25.228Z' },
        };

        const obj = model.modify(src, {
          $unset: {
            title: true,
            'meta.date': true,
            owners: true,
        ...
        assert.strictEqual(undefined, obj.title);
        assert.strictEqual('kandinsky', obj.def);
        assert.strictEqual('me', obj.author);
        assert.strictEqual(true, Array.isArray(obj.numbers));
        assert.equal(obj.meta.date, undefined);
        assert.equal(obj.numbers.length, 0);
        assert.equal(obj.owners, undefined);
        assert.strictEqual(true, Array.isArray(obj.comments));
      ...
      const src = {
        title: 'top',
        comments: [
          { title: 'a', body: 'body', comments: [] },
          {
            title: 'b',
            body: 'body',
            comments: [{ title: 'c', body: 'body', comments: [{ title: 'd', body: 'body' }] }],
          },
        ],
      };

      it('Should setionValue work as expected', () => {
        let res = model.setionDotValue(src, 'title');
        assert.deepEqual(res.obj, src);

        res = model.setionDotValue(src, 'comments.title');
        assert.deepEqual(res.obj, src.comments);

        res = model.setionDotValue(src, 'comments.comments');
        assert.deepEqual(res.obj, src.comments);

        res = model.setionDotValue(src, 'comments.comments.title');
        assert.deepEqual(res.obj, [{ title: 'c', body: 'body', comments: [{ title: 'd', body: 'body' }] }]);

        res = model.setionDotValue(src, 'comments.comments.comments.title');
        assert.deepEqual(res.obj, [{ title: 'd', body: 'body' }]);
      });

      it('When unsetting deeply nested array, should work as expected', () => {
        const obj = model.modify(src, {
          $unset: {
            'comments.title': true,
        ...
        assert.strictEqual(obj.title, 'top');
        assert.strictEqual(true, Array.isArray(obj.comments));
        obj.comments.forEach((comment) => {
    ...
    describe('$inc modifier', () => {
      ...
      it('Works with nested arrays', () => {
        const obj = { nested: [{ name: 'name' }] };
        let modified;

        modified = model.modify(obj, { $inc: { 'nested.$.notexist': 2 } });
        _.isEqual(modified, { nested: [{ name: 'name', notexist: 2 }] }).should.equal(true);
      });
    }); // End of '$inc modifier'
    ...
    describe('$push modifier', () => {
      ...
      it('Can $push to nested fields in an array', () => {
        const obj = {
          nested: [
            { name: 'old1', sub: [] },
            { name: 'old2', sub: [] },
        ...
        const updateQuery = { $push: { 'nested.$.sub': 'subnew' } };
        const modified = model.modify(obj, updateQuery);

        assert.deepEqual(modified, {
          nested: [
            { name: 'old1', sub: ['subnew'] },
            { name: 'old2', sub: ['subnew'] },
          ],
        });
      });
    ...
    describe('$pull modifier', () => {
      ...
      it('Works with nested arrays', () => {
        ...
        modified = model.modify(obj, { $pull: { nested: { name: 'name' } } });
        _.isEqual(modified, { nested: [] }).should.equal(true);
      });
  ...
  describe('Querying', () => {
    ...
    describe('Comparing on arrays', () => {
      it('Can perform a direct array match', () => {
        model
          .match({ planets: ['Earth', 'Mars', 'Pluto'], something: 'else' }, { planets: ['Earth', 'Mars'] })
          .should.equal(false);
        model
          .match({ planets: ['Earth', 'Mars', 'Pluto'], something: 'else' }, { planets: ['Earth', 'Mars', 'Pluto'] })
          .should.equal(true);
        model
          .match({ planets: ['Earth', 'Mars', 'Pluto'], something: 'else' }, { planets: ['Earth', 'Pluto', 'Mars'] })
          .should.equal(false);
      });
      ...
      it('Can use $size several times in the same matcher', () => {
        model
          .match({ childrens: ['Riri', 'Fifi', 'Loulou'] }, { childrens: { $size: 3, $size: 3 } })
          .should.equal(true);
        model
          .match({ childrens: ['Riri', 'Fifi', 'Loulou'] }, { childrens: { $size: 3, $size: 4 } })
    ...
    describe('Comparison operator $where', () => {
      ...
      it('Should be able to do the complex matching it must be used for', () => {
        ...
        model
          .match({ firstName: 'John', lastName: 'Doe', email: 'john.doe@gmail.com' }, { $where: checkEmail })
          .should.equal(true);
        model
          .match({ firstName: 'john', lastName: 'doe', email: 'john.doe@gmail.com' }, { $where: checkEmail })
          .should.equal(true);
        model
          .match({ firstName: 'Jane', lastName: 'Doe', email: 'john.doe@gmail.com' }, { $where: checkEmail })
          .should.equal(false);
        model
          .match({ firstName: 'John', lastName: 'Deere', email: 'john.doe@gmail.com' }, { $where: checkEmail })
          .should.equal(false);
    ...
    describe('Array fields', () => {
      ...
      it('Can mix queries on array fields and non array filds with array specific operators', () => {
        model
          .match({ uncle: 'Donald', nephews: ['Riri', 'Fifi', 'Loulou'] }, { nephews: { $size: 2 }, uncle: 'Donald' })
          .should.equal(false);
        model
          .match({ uncle: 'Donald', nephews: ['Riri', 'Fifi', 'Loulou'] }, { nephews: { $size: 3 }, uncle: 'Donald' })
          .should.equal(true);
        model
          .match({ uncle: 'Donald', nephews: ['Riri', 'Fifi', 'Loulou'] }, { nephews: { $size: 4 }, uncle: 'Donald' })
          .should.equal(false);

        model
          .match({ uncle: 'Donals', nephews: ['Riri', 'Fifi', 'Loulou'] }, { nephews: { $size: 3 }, uncle: 'Picsou' })
          .should.equal(false);
        model
          .match({ uncle: 'Donald', nephews: ['Riri', 'Fifi', 'Loulou'] }, { nephews: { $size: 3 }, uncle: 'Donald' })
          .should.equal(true);
        model
          .match({ uncle: 'Donald', nephews: ['Riri', 'Fifi', 'Loulou'] }, { nephews: { $size: 3 }, uncle: 'Daisy' })
          .should.equal(false);
      });
    });
  }); // ==== End of 'Querying' ==== //
